...
<a href="{{ route('home') }}" target="_blank" rel="noopener noreferrer"
    class="sidebar-link group flex items-center justify-between rounded-xl px-3 py-2.5 text-sm font-medium text-sidebar-text hover:bg-sidebar-hover hover:text-sidebar-text-hover transition-all duration-200">
    ...
    <svg class="h-4 w-4 shrink-0 text-sidebar-text/50 group-hover:text-sidebar-text-hover transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div x-data="{ open: {{ request()->routeIs('admin.settings.*') ? 'true' : 'false' }} }">
    <button type="button"
            @click="open = !open"
            @class([
                'sidebar-link flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-200 cursor-pointer',
                'bg-white/15 text-white font-semibold' => request()->routeIs('admin.settings.*'),
        ...
        <svg class="h-4 w-4 shrink-0 transition-transform duration-200" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
        </svg>
    </button>
    <div x-show="open" x-cloak class="mt-1 ml-4 border-l border-white/10 pl-3 space-y-1">
        <a href="{{ route('admin.settings.index', ['tab' => 'info']) }}"
           @class([
               'flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition-all duration-200',
               'bg-primary text-white font-semibold shadow-xs' => request()->routeIs('admin.settings.*') && request()->get('tab', 'info') === 'info',
               'text-sidebar-text/80 hover:bg-sidebar-hover hover:text-white' => !(request()->routeIs('admin.settings.*') && request()->get('tab', 'info') === 'info'),
           ])>
        ...
        <a href="{{ route('admin.settings.index', ['tab' => 'contact']) }}"
           @class([
               'flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition-all duration-200',
               'bg-primary text-white font-semibold shadow-xs' => request()->routeIs('admin.settings.*') && request()->get('tab') === 'contact',
               'text-sidebar-text/80 hover:bg-sidebar-hover hover:text-white' => !(request()->routeIs('admin.settings.*') && request()->get('tab') === 'contact'),
           ])>
            <span class="h-1.5 w-1.5 rounded-full bg-current opacity-70"></span>
            Kontak & Sosial Media
        </a>
        ...
        <a href="{{ route('admin.settings.index', ['tab' => 'system']) }}"
           @class([
               'flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition-all duration-200',
               'bg-primary text-white font-semibold shadow-xs' => request()->routeIs('admin.settings.*') && request()->get('tab') === 'system',
               'text-sidebar-text/80 hover:bg-sidebar-hover hover:text-white' => !(request()->routeIs('admin.settings.*') && request()->get('tab') === 'system'),
           ])>
            <span class="h-1.5 w-1.5 rounded-full bg-current opacity-70"></span>
            Sistem
        </a>
...
<a href="{{ route('admin.company-profile.index') }}"
    @class([
        'sidebar-link flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-200',
        'bg-primary text-white shadow-sm' => request()->routeIs('admin.company-profile.*'),
        'text-sidebar-text hover:bg-sidebar-hover hover:text-sidebar-text-hover' => !request()->routeIs('admin.company-profile.*'),
    ])>
    <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
    </svg>
