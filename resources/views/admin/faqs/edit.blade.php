...
<x-layouts::admin :title="$title">
    ...
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-text-secondary">Perbarui informasi pertanyaan "{{ $faq->question }}".</p>
    </div>
    ...
    <div class="max-w-4xl xl:max-w-5xl">
        <div class="rounded-xl border border-border bg-card p-6 shadow-sm">
            <form method="POST" action="{{ route('admin.faqs.update', $faq) }}">
                ...
                {{-- Pertanyaan --}}
                <div class="mb-5">
                    <label for="question" class="mb-1.5 block text-sm font-medium text-text-primary dark:text-white">
                ...
                {{-- Jawaban --}}
                <div class="mb-5">
                    <label for="answer" class="mb-1.5 block text-sm font-medium text-text-primary dark:text-white">
                    ...
                    <textarea id="answer"
                              ...
                              class="w-full rounded-lg border {{ $errors->has('answer') ? 'border-red-300 focus:border-red-500 focus:ring-red-500' : 'border-border focus:border-primary focus:ring-primary' }} bg-card px-4 py-2.5 text-sm text-text-primary placeholder-text-muted outline-hidden ring-0 transition-colors">{{ old('answer', $faq->answer) }}</textarea>
                    @error('answer')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 gap-5 lg:grid-cols-2 mb-6">
                    {{-- Urutan --}}
                    <div>
                        <label for="sort_order" class="mb-1.5 block text-sm font-medium text-text-primary dark:text-white">
                            Urutan
                        </label>
                        <input type="number"
                               id="sort_order"
                    ...
                    {{-- Status --}}
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-text-primary dark:text-white">
                            Status
                        </label>
