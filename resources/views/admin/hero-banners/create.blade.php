...
<x-layouts::admin :title="$title">
    ...
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-text-secondary">Tambahkan hero banner baru untuk halaman depan.</p>
    </div>
                ...
                <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select id="text_position"
                            ...
                            <option value="left" {{ old('text_position') == 'left' ? 'selected' : '' }}>Kiri</option>
                            <option value="center" {{ old('text_position', 'center') == 'center' ? 'selected' : '' }}>Tengah</option>
                            <option value="right" {{ old('text_position') == 'right' ? 'selected' : '' }}>Kanan</option>
                        </select>
                        @error('text_position')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
...
<div>
                        ...
                        <select id="primary_button_link"
                            ...
                            @foreach (\App\Enums\HeroButtonTarget::options() as $value => $label)
                                <option value="{{ $value }}" {{ old('primary_button_link') == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
...
<div>
                        ...
                        <select id="secondary_button_link"
                            ...
                            <option value="">-- Pilih Tujuan --</option>
                            @foreach (\App\Enums\HeroButtonTarget::options() as $value => $label)
                                <option value="{{ $value }}" {{ old('secondary_button_link') == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
                    ...
                    <div>
                        <label for="image" class="mb-1.5 block text-sm font-medium text-text-primary dark:text-black">
                            Gambar Latar <span class="text-red-500">*</span>
                        </label>
                        <input type="file"
                               id="image"
                               name="image"
                        ...
                        @error('image')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-text-muted">Format: JPG, PNG, WebP. Maks: 5MB. Rekomendasi: 1920x1080px.</p>
                    </div>

                    {{-- Opacity Overlay --}}
                    <div>
                        <label for="overlay_opacity" class="mb-1.5 block text-sm font-medium text-text-primary dark:text-black">
                            Opacity Overlay: <span class="font-semibold text-primary" x-text="form.overlay_opacity + '%'">40%</span>
                        </label>
                        <input type="range"
                               id="overlay_opacity"
        ...
        function heroForm() {
            return {
                form: {
                    ...
                    primary_button_text: '{{ old('primary_button_text', '') }}',
                    primary_button_link: '{{ old('primary_button_link', '') }}',
                    secondary_button_text: '{{ old('secondary_button_text', '') }}',
                    secondary_button_link: '{{ old('secondary_button_link', '') }}',
                    text_position: '{{ old('text_position', 'center') }}',
                    overlay_opacity: {{ old('overlay_opacity', 40) }},
