...
<x-layouts::admin :title="$title">
    ...
    <div class="mb-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-text-primary dark:text-black">Hero Banner</h2>
                <p class="mt-1 text-sm text-text-secondary">Kelola banner utama halaman depan website.</p>
            </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-border bg-bg-secondary/50">
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-text-muted">Hero</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-text-muted">Posisi Teks</th>
                        ...
                        <tr class="transition-colors hover:bg-bg-secondary/30">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    @else
                                        <div class="flex h-14 w-20 shrink-0 items-center justify-center rounded-lg bg-bg-secondary text-text-muted">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    {{-- Lihat --}}
                                    <a href="{{ route('admin.hero-banners.show', $hero) }}"
                                    ...
                                    {{-- Toggle Status --}}
                                    <form method="POST" action="{{ route('admin.hero-banners.toggle-status', $hero) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                                class="inline-flex items-center gap-1.5 rounded-lg border border-border bg-card px-3 py-1.5 text-xs font-medium {{ $hero->status === 'active' ? 'text-red-600 hover:bg-red-50' : 'text-emerald-600 hover:bg-emerald-50' }} transition-colors"
                                                title="{{ $hero->status === 'active' ? 'Nonaktifkan' : 'Aktifkan' }}">
                                            @if ($hero->status === 'active')
                                                <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                                </svg>
                                    ...
                                    <button type="button"
                                            x-data
                                            x-on:click="$dispatch('open-modal', 'delete-hero-{{ $hero->id }}')"
                                    ...
                                    <div x-data="{ show: false }"
                                         x-on:open-modal.window="if ($event.detail === 'delete-hero-{{ $hero->id }}') show = true"
                                         x-show="show"
                                         x-cloak
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    @if ($search || $statusFilter)
                                        <p class="text-sm font-medium text-text-primary dark:text-white">Hero banner tidak ditemukan</p>
                                        <p class="mt-1 text-xs text-text-muted">Tidak ada hero banner yang cocok dengan filter yang dipilih.</p>
                                    @else
                                        <p class="text-sm font-medium text-text-primary dark:text-white">Belum ada hero banner</p>
                                        <p class="mt-1 text-xs text-text-muted">Mulai dengan menambahkan hero banner baru.</p>
                                    @endif
                                </div>
                            </td>
                        </tr>
