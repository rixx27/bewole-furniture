        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div class="relative w-full max-w-md rounded-xl bg-card p-6 shadow-2xl border border-border"
                ...
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-semibold text-text-primary dark:text-white">Ubah Status Pembayaran</h3>
                ...
                <div class="mb-4 rounded-lg border border-border bg-bg-secondary/50 p-3">
                    ...
                    <div class="mt-2 flex items-center justify-between text-sm">
                        ...
                        <span class="font-semibold text-text-primary dark:text-white">{{ $order->formatted_total_price }}</span>
                    </div>
                </div>
                ...
                <form wire:submit="updatePayment">
                    ...
                    <div class="mb-4">
                        <label class="mb-2 block text-sm font-medium text-text-primary dark:text-white">Pilih Status Pembayaran</label>
                        <div class="space-y-2">
                            @php
                                $paymentStatuses = [
                            ...
                            @foreach ($paymentStatuses as $status)
                                <label class="flex cursor-pointer items-center gap-3 rounded-lg border border-border p-3 hover:bg-bg-secondary transition-colors {{ $payment_status === $status->value ? 'border-primary bg-primary/5' : '' }}">
                                    <input type="radio" wire:model="payment_status" name="payment_status" value="{{ $status->value }}" class="h-4 w-4 text-primary border-border focus:ring-primary">
                                    <div>
