<div>
    ...
    @if ($order)
        ...
        <div class="mx-auto mt-10 max-w-5xl space-y-8">
            ...
            {{-- 3. Detail Grid: Product, Payment & Timeline --}}
            <div class="grid gap-8 lg:grid-cols-3">
                
                {{-- Left: Payment Management & Status Timeline (2 Cols) --}}
                ...
                <div class="space-y-6">
                    ...
                    <div class="rounded-3xl border border-wood-border/60 bg-white p-6 shadow-sm">
                        <div class="flex items-center justify-between border-b border-wood-border/40 pb-3 mb-4">
                            <h3 class="text-sm font-bold text-wood-text">Informasi Produk</h3>
                            <span class="text-xs font-semibold text-wood-muted">
                                {{ $order->items && $order->items->count() > 0 ? $order->items->count() : 1 }} Produk
                            </span>
                        </div>

                        @if ($order->items && $order->items->count() > 0)
                            <div class="divide-y divide-wood-border/40 space-y-3">
                                @foreach ($order->items as $item)
                                    <div class="flex gap-3 pt-3 first:pt-0">
                                        <div class="h-14 w-14 shrink-0 overflow-hidden rounded-xl bg-wood-light/40 border border-wood-border/40">
                                            ...
                                            @else
                                                <div class="flex h-full w-full items-center justify-center text-wood-muted">
                                                    <svg class="h-6 w-6 stroke-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div class="flex-1 min-w-0 text-xs">
                                            <h4 class="font-semibold text-wood-text truncate">{{ $item->product?->name ?? 'Produk' }}</h4>
                                            <p class="text-wood-muted mt-0.5">{{ $item->quantity }} pcs × Rp {{ number_format($item->unit_price, 0, ',', '.') }}</p>
                                            <p class="font-bold text-wood-primary mt-0.5">Rp {{ number_format($item->total_price, 0, ',', '.') }}</p>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <div class="rounded-3xl border border-wood-border/60 bg-white p-6 shadow-sm space-y-4 text-xs">
                        ...
                        <div>
                            ...
                            @if ($order->customer_phone)
                                <span class="text-wood-muted text-[11px] block mt-0.5">{{ $order->customer_phone }}</span>
                            @endif
                        </div>
                        ...
                        {{-- WhatsApp Shipping & Ongkir Notice --}}
                        <div class="rounded-2xl border border-amber-200 bg-amber-50/70 p-3.5 space-y-1.5 text-xs">
                            <div class="flex items-center gap-1.5 font-bold text-amber-900">
                                ...
                                <span>Koordinasi Ongkir & Pengiriman</span>
                            </div>
                            <p class="text-amber-800/90 leading-relaxed text-[11px]">
                                Pengiriman meubel diproses via armada truk ekspedisi lokal Jepara. Estimasi biaya ongkir dan jadwal tiba dikoordinasikan langsung melalui WhatsApp.
                            </p>
                        </div>
                        ...
                        @if ($cleanOrderWa)
                            <a
                                href="https://wa.me/{{ $cleanOrderWa }}?text=Halo%20Bewole%20Furniture%2C%20saya%20ingin%20koordinasi%20pengiriman%20dan%20ongkir%20untuk%20pesanan%20nomor%20%23{{ $order->order_code }}"
                                target="_blank"
                            ...
                            >
                                ...
                                Koordinasi Ongkir via WhatsApp
                            </a>
                        @endif
                    </div>
                </div>
